...
التاريخ: 2026-09-22 (النسخة 4، بعد ثلاث جولات مراجعة مستقلة). الحالة: **تصميم مقترح لإعادة بناء كاملة**، غير منفذ وغير مختبر. لا يلتزم بهيكل الكود الحالي؛ القسم 21 يحدد ما يُنقل منه. يكمّل [المعمارية المستهدفة](architecture.md) و[الأمن والاعتمادية](security-and-reliability.md)، ويحل محل أي تصميم داخلي للمحرك يخالفه بعد اعتماده. سجل المراجعات في القسم 23.
...
| الأداء | تشبع 1 Gbps بملف واحد على الجهاز المرجعي؛ المعالج أثناء النقل ≤ 10% من **نواة واحدة** لكل 100 MB/s، **دون** مرحلة التحقق (ميزانيتها في §10.5) |
...
> **حالة التنفيذ (2026-09-23):** هذا القسم **تصميم مستهدف**، وما نُفِّذ منه أقل. المنفَّذ: مقبس Unix في مجلد 0700 مملوك للمستخدم مع `SO_PEERCRED` وفحص ملكية من العميل قبل إرسال أي بايت؛ وأنبوب Windows بـ`FIRST_PIPE_INSTANCE` و`PIPE_REJECT_REMOTE_CLIENTS` و`SECURITY_IDENTIFICATION` من العميل. **غير المنفَّذ: اسم الأنبوب بـSID وجلسة، وDACL للمالك تستبعد AppContainer وLow-IL، وتحقق العميل من مالك الأنبوب.** لذلك ما يلي في هذا القسم عن Windows وصف لما يجب أن يكون، والعبارة «انتحال الاسم يسبب DoS لا اختراق» **لا تصحّ اليوم** لأن حمايتها الفعلية (تحقق المالك) غير موجودة. التفصيل ومسار المعالجة في [سطح التحكم](enterprise-ipc.md).
...
- العملاء في sandbox (AppContainer، Low-IL، Flatpak) لا يصلون بتصميم الـDACL/الصلاحيات؛ يُختبر ذلك.
...
| مستخدم آخر على نفس الجهاز | **مُخفَّف على Unix** (ملكية وصلاحيات وSO_PEERCRED). **غير مُخفَّف على Windows:** بلا DACL ولا تحقق مالك، انتحال اسم الأنبوب يسرّب ما يرسله العميل (ومنه رابط موقّع) لا مجرد تعطيل. رفض الوجهات المشتركة غير منفَّذ؛ المنفَّذ: رفض الوجهة داخل شجرة المحرك وخارج جذر التنزيل (بمقارنة مسارات يحسمها نظام الملفات لا هجاؤها)، ورفض أسماء الأجهزة والامتدادات التنفيذية أينما وقعت والنقطة/المسافة في النهاية ومحارف قلب الاتجاه وفاصل البدائل `:`. **ليس كل §16.4**: كشف الامتداد المزدوج بمعناه العام غير منفَّذ |
| تطبيقات sandboxed بنفس الحساب (AppContainer، Low-IL، Flatpak) | **مقترح لا منفَّذ:** المنع يحتاج DACL صريحة لم تُكتب بعد، ولا اختبار له. على Windows اليوم تستطيع عملية بنفس المستخدم — ومنها sandboxed تقرأ — شغل نسخ الأنبوب |
