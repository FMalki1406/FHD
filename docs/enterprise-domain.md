...
مرجع مراجعة الحالة: الكود الملتزم `bcbc04266d03af694d9c127e22c8b31bb25b0710`؛ لا يشمل تغييرات العمل غير الملتزمة ولا تشغيل اختبارات جديدًا. `fhd-domain` يقود حالات المنسق الجديد في `fhd-engine`، ويربط جذر التركيب النقل والتخزين والاستعادة عبر منافذ التطبيق. لا I/O في الدومين نفسه. المحرك القديم `fhd-transfer` وحالاته المنفصلة باقيان؛ لا يدعي هذا اكتمال الانتقال أو التأهيل. نتائج التحقق في [سجل التنفيذ](execution-status.md).
...
`discard_checkpoint`/`Job::abandon_checkpoint` مسار متصل موثوق، لا أمر عميل: لا يستعمل بعد نتيجة commit ملتبسة إلا بعد إيقاف استخدام التذكرة القديمة ومصالحة القرص والمستودع وإعادة التحقق من البيانات. إبطال التذكرة لا يتراجع عن معاملة قاعدة ربما نجحت. المصالحة وإعادة بناء extents منفذتان خارج الدومين في المنسق والمستودع؛ لا يؤدي الدومين الخالص I/O بنفسه. اختيار تجاهل المصالحة واستدعاء هذه الواجهة مباشرة مخالفة للعقد، ولا يستطيع دومين خالص اكتشافها.
...
المسار الصحيح للتطبيق: `decide` دون تعديل → حفظ الحدث والتغييرات المرتبطة بمعاملة → `apply` بعد الإقرار. apply يرفض حدثًا مكررًا أو خارج الترتيب أو من مهمة/جيل آخر، ولا يعدل الحالة عند الرفض. `handle` اختصار متعمد للمحاكاة يطبق فورًا؛ لا يستخدم مباشرة على الحالة المنشورة قبل commit. الأحداث خاصة الإنشاء؛ المستودع يعيد بناء المهمة عبر Job::restore من الإسقاط والنطاقات المتينة، ويظل خارج الدومين.
...
SourceChanged وIntegrity لا يسمحان Resume عاديًا يحتفظ بالأجزاء القديمة؛ يحتاجان ReplaceRepresentation من حالة متوقفة، مما يرفع الجيل ويزيل خريطة العمل من نموذج المهمة. التحقق من strong ETag وربط النطاق به منفذ في محول HTTP والمنسق؛ هذا لا يعني اكتمال نموذج Representation الغني بكل سياسات سياق الطلب.
...
هذه اختبارات بلا IO ولا بديل لاختبارات انهيار المحولات، ولا تدعي تشغيل proptest أو fuzzing. SegmentMap يستخدم Vec وخوارزميات محدودة لكنها خطية، وقرار Job يأخذ لقطة داخلية؛ لا دليل بعد على ملاءمتها لأقصى أحجام الخريطة أو أهداف CPU. يلزم benchmark قبل تحويلها لمسار كثيف. استعادة extents والعامل المستقل والكاتب وBufferPool وIPC منفذة في طبقاتها، لا داخل الدومين. ما زالت Representation الغنية وgroup commit بين المهام والسياسات والتنقيح المتكامل غير مكتملة.
