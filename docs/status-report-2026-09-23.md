...
| **تطبيق معزول بنفس الحساب** (sandbox / AppContainer) | **Windows: لا** — علامة النزاهة الإلزامية تمنع الفتح. **Linux: نعم** — لا مكافئ منفَّذ. **وهذه هي الثغرة الحقيقية الوحيدة** |
...
- **الأثر الحقيقي:** على **Linux** فقط، تطبيق معزول يصل إلى المقبس يستطيع الإدراج والإلغاء والإيقاف وقراءة قائمة المهام.
- **الدليل:** `crates/bins/daemon/src/service.rs:507-552` — `Add` وحده يمر بـ`AddDownload`.
- **المعالجة:** مكافئ العزل على Linux أولًا؛ ومنفذ `Authorizer` على كل أمر ثانيًا (دفاع في العمق، لا حاجز).
- **يمنع الإطلاق؟ على Linux نعم.** وليس شرطًا لاكتمال الترخيص كما كتبت خطأً.
